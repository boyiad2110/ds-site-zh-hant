...
| 段落標記 | `**效果**：` `**觸發**：` `**花費 N 怒火**：` |
| **括號內句尾不加句號** | `(Quick Build: Paragon.)` → 「（推薦選項：典範教團）」——**省略**原文的句號 |
| **內文提及招式名時加【】** | 「戰術家的【快出手】招式」，不是「戰術家的快出手招式」 |

> Notion 舊譯的 🔍📐⏩🎯 欄位圖示是 Notion 的呈現手段。新站這些是結構化欄位，改由 `<Glyph>` 元件渲染，**不進資料**。

### 6.1 招式名稱的驚嘆號（2026-07-29 裁定）

舊譯有些招式名帶驚嘆號（「快出手！」）。擁有者查對原版規則書後確認：
**原文不帶驚嘆號的版本較多，故中文一律省略驚嘆號。**

⚠️ 這只針對**舊譯自行添加**的驚嘆號。原文本身就有驚嘆號的招式名（`Halt Miscreant!`、
`Back Blasphemer!`、`Behold a Shield of Faith!` 等）**照原文保留**——那是官方的命名風格，不是贅字。

判斷方式：看 `data/canon/` 該條目的 `name` 有沒有 `!`，有就留、沒有就不加。
