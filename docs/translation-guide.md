...
| 全形標點 | ：、（）。「」 |
| **多欄階層合併用全形逗號** | 舊譯的階層表格分欄（`2 神聖傷害 │ 推動 1`），正典是單一字串（`2 holy damage; push 1`），中文層合併為一個 `text`，**欄間一律以「，」分隔**：「2 神聖傷害，推動 1」。**不用「；」**，理由：舊譯儲存格內部本就以「，」串接，且中文全形分號在規則短句中過重。2026-07-29 擁有者確認 |
