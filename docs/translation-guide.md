...
存法：`text` 只存無條件結果；`potency` 存 `{ characteristic, level, effect }`，其中 `effect` 是條件式結果；`raw` 保存完整原文。中文層用同階層的 `potencyEffect` 對應 `potency.effect`。**呈現格式見 §6.1b**（2026-07-30 改裁：兩者合併一行呈現，不再是「先 text 再另起一段標示 potencyEffect」的做法）——資料仍分開存，只是渲染時合併，不可只靠顏色暗示兩者的關聯。
...
### 4.3.3 條件式後續效果不是追加花費（⚠️ 2026-07-30 撤銷，見下）

若費用是在招式結算後、另一個規則事件發生時才可選擇支付，曾使用 `conditionalEffects`：
...
> ⚠️ **本結構已於 2026-07-30 撤銷（TI-28）。** 唯一使用者【當場拘捕】的實際呈現是一個獨立方框，擁有者檢視後認為原版規則書根本沒有把這段話獨立出來——它就寫在同一個 `Effect:` 段落裡，改回單一 `effect` 陣列即可，不需要 `trigger`／`optional`／`cost` 這層結構。**保留本節說明是為了讓未來的 Agent知道這個型別為什麼存在過、為什麼被撤銷**，不是可以直接沿用的作法。若日後真的遇到「費用要等特定事件發生才可選擇支付」的招式，先問擁有者要不要重開這個型別，不要逕自比照舊決定恢復使用。
...
#### `willing` / `willingly` —— 副詞與形容詞刻意用不同的中文（2026-07-29 裁定）

| 原文 | 中文 | 出現在 |
|---|---|---|
| `One willing creature` | 1 個**自願的**生物 | 目標欄的修飾語 |
| `willingly moves` | **主動**移動 | 效果文字裡的移動限定 |

**不強求同一英文字的兩種詞性使用相同中文。** 兩種譯法都保留「自願」的概念，
但各自的中文更自然（「目標每自願移動 1 格」讀起來拗口）。

兩者管的是不同的事：

| 片語 | 管什麼 |
|---|---|
| `One willing creature`＝1 個**自願的**生物 | 目標**是否同意被指定**（與移動無關） |
| `willingly moves`＝**主動**移動 | 區分**自行移動**與**被強制移動** |

⚠️ `willingly` 是**關鍵限定詞**——它決定招式在被推動、被拉動時觸不觸發，
不是語氣詞，不得省略。紀錄見 `data/translation-issues.json` TI-20。
...
| 全形標點 | ：、（）。「」 |
| **多欄階層合併用全形分號**（2026-07-30 改裁，見下） | 舊譯的階層表格分欄（`2 神聖傷害 │ 推動 1`），正典是單一字串（`2 holy damage; push 1`），中文層合併為一個 `text`，**欄間一律以「；」分隔**：「2 神聖傷害；推動 1」 |
...
> ⚠️ **2026-07-30 撤銷 2026-07-29 的逗號裁決。** 原判斷（舊譯儲存格本就用逗號串接、全形分號在短句中過重）在紙面上合理，但擁有者實際檢視 M0 驗收頁面的呈現後，認為分號更貼近原版規則書本身的標點（原文就是用 `;` 分隔，如 `2 holy damage; push 1`），逗號反而弱化了子句邊界。正式紀錄見 `data/translation-issues.json` TI-29。**教訓：格式規則最終要看實際呈現效果，紙上判斷可能不夠。**

### 6.1a 招式效果段的呈現原則（2026-07-30 裁定，同日第二輪修訂）

**`followUpActions`**：渲染時不做成獨立的標題方框（帶邊框、獨立標題、chip 徽章），直接接在「效果」標題下方連續呈現——原版規則書把後續戰術寫在同一個 `Effect:` 段落底下，只是段落中間出現引導句＋條列選項＋限制句。

**`extraCosts`**（⚠️ 第二輪改回）：第一輪曾把它也折進「效果」段落內文（粗體「花費 N 點怒火：」引言），擁有者看過實際呈現後不喜歡，改回**跟「觸發」「效果」平行的獨立段落**：標題就是「花費 N 點怒火」（無需冒號或粗體前綴），底下接一段描述文字，跟 Trigger／Effect 的視覺份量一致，但同樣不加邊框或 chip 徽章。**兩者的差別**：`followUpActions` 原文就寫在 Effect 段落裡（一句話帶過「你可以花費…」），`extraCosts` 原文是「Spend N Wrath:」開頭的獨立句子——分開處理才貼近書本各自的排版邏輯。

`conditionalEffects` 這個結構已撤銷（見 TI-28）——它是本輪之前新增的型別，唯一使用者【當場拘捕】已改回單一 `effect`。若日後遇到真正需要「稍後才可選擇支付」語意的新招式，**先問擁有者要不要重開這個型別，不要逕自比照舊決定恢復使用**。

> ⚠️ **`trigger` 欄位曾經完全沒有渲染。** 2026-07-30 第二輪驗收才發現：`ability.censor.my-life-for-yours` 是唯一有 `trigger` 的招式，但網站從來沒有把它顯示出來——不是這輪改壞的，是原本就沒接這條路徑。已補上，順序是「觸發 → 效果 → 花費 N 點怒火」。**教訓：新增資料型別時，逐一確認每個非空欄位都有對應的渲染路徑，不能只看測試綠燈**（`verify-zh-structure` 只驗證資料完整，不驗證有沒有被畫出來——見 NEXT-SESSION.md「驗證邊界二」）。

### 6.1b 威力擲骰的效力呈現（2026-07-30 裁定，同日第二輪修訂樣式）

`tiers[].potency` 與 `potencyEffect` 不再各自成一行、用虛線與方框隔開，改成接在同一階層的傷害文字後面，**一行呈現**，格式仿照原文本身的符號寫法：

```
{text}；<徽章>{屬性} < {效力等級}</徽章>，{potencyEffect}
```

例：「7 + `力量`神聖傷害；`氣場 < 強`（黑底白字徽章），目標陷入緩速（豁免解除）」。

分號分開「基本結果」與「效力結果」；效力標記本身**用徽章樣式**呈現（黑底白字，仿原版規則書 `P<STRONG` 這類符號的視覺份量），第一輪曾改成單純加粗文字，擁有者看過實際呈現後認為份量不夠，改回徽章。徽章後面接逗號再接效果句——這個標點順序直接對應原文的 `7 + M holy damage; P<STRONG, slowed (save ends)`。

`tiers[].text` 與 `potencyEffect` **仍然各自存成獨立欄位**，只有 renderer 呈現時合併——這一點沒有變，資料層仍要能單獨取出「無條件結果」與「效力結果」（§2.1、§4.3.0 的資料模型不受影響，只是呈現方式變了）。
...
- **`證據` 欄**：🔴 high 項目必附**算繪圖的檔案路徑**（不嵌圖，避免報告膨脹）。

  **解析度先用 150dpi，看得清楚就直接引用，不要再裁 300dpi**（擁有者 2026-07-29 裁定）。
  150dpi 全頁約 2,800 tokens、300dpi 全頁約 11,000；已經讀得出來還去放大，
  等於白花一次判讀成本。**只有 150dpi 真的辨識不出來時**才裁切放大。
...
#### 8.4.1 兩條硬性規則（2026-07-29 加入）

第三批報告（10 個招式）長達 500 多行，其中大半在證明「沒問題」，而且摘要的數字有三處是錯的
（⚪ 項寫 8 實為 11、術語寫 35 實為 56、測試數在三處寫成 15／19／15）。
兩條規則直接對應這兩個毛病：

**(1) 任何數字都必須從清單長出來，不准用寫的。**

報告裡出現的每一個計數，都要附**可以自己加一遍的來源**——逐條清單、逐欄表格，
或一行可重跑的指令。寫不出來源就不要寫那個數字。

必附的是**逐條目欄位帳**，格式如下（第三批報告的版本可直接抄）：

| 招式 | 欄位數 | exact | acceptable（涵蓋欄位） | missing |
|---|---|---|---|---|

並在表後寫明**維度二數欄位、維度三數問題**，兩者不會相等（一個問題可涵蓋多欄位），
再把兩邊對帳一次。**只有帳對得起來，數字才可信。**

**(2) 相符的欄位不逐欄展開。**

全部相符時寫一行「N 個欄位逐欄相符」，佐證留在上面那張帳裡即可。
需要展開的只有三類：**需要裁決的項目**、**新出現的結構**、**與前批不同的處理方式**。

> 這兩條的目的不是省字，是**讓錯誤沒地方躲**。
> 攤開的清單可以被複驗；憑印象寫的摘要不行。
...
| 2026-07-29 | Fire Weakness | glossary | **火焰弱點** | — | 擁有者裁決（zh-batch3 UT-B3-1）：確立下方構詞規則，與既有 `Damage Weakness ＝傷害弱點` 同構 |

**未晉升 `approved` 的詞不得作為 CI 規則，也不得用於自動改寫舊譯。**

### A.1 構詞規則

構詞規則**不是術語**，是「怎麼組出術語」的規則。組出來的每個詞仍須逐一建立 glossary 詞條並經裁決，
規則只保證組法一致、不必每次重新討論譯法。

| 日期 | 規則 | 適用範圍 | 已依此建立的詞條 |
|---|---|---|---|
| 2026-07-29 | **`<傷害類型> Weakness` ＝「<傷害類型>弱點」** | **僅限傷害類型弱點這個規則 sense。** 刻意**不**建立無範圍的 `Weakness ＝弱點` 詞條——那會套用到一般語境（如敘述文字裡的「弱點」），擴大裁決範圍 | `Damage Weakness ＝傷害弱點`（2026-07-28）、`Fire Weakness ＝火焰弱點`（2026-07-29）。日後酸蝕、寒冷等依同規則逐一建立 |
