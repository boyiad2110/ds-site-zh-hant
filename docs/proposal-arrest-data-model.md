...
> **狀態：已撤銷（2026-07-30）。** 本提案曾於 2026-07-29 被採用並實作為 `conditionalEffects` 結構，
> 但擁有者於 2026-07-30 M0 驗收時檢視實際網站呈現後撤銷——原版規則書這句話本就寫在
> 同一個 `Effect:` 段落裡，不是獨立方框內容，改回單一 `effect` 陣列即可。
> 正式裁決紀錄見 `data/translation-issues.json` TI-28；本文保留供稽核，
> 說明這個結構當初為什麼被提出、後來為什麼被撤銷——**不是可以直接沿用的作法**。
